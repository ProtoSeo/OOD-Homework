...
Number *Complex::add(Number * num) {
	...
	if(num->types() == INTEGER){
		...
		this->set_val(this->real()+temp->val(),this->imag());
		delete temp;
		return this;
	}
		...
		this->set_val(this->real()+temp->real(),this->imag()+temp->imag());
		delete temp;
		return this;
	}
	else{
...
Number *Complex::mul(Number * num) {
	...
	if(num->types() == INTEGER){
		Integer* temp = dynamic_cast<Integer*>(num);
		this->set_val(this->real()*temp->val(),this->imag()*temp->val());
		delete temp;
		return this;
	}
	else if(num->types() == FLOAT){
		...
		Complex* temp = dynamic_cast<Complex*>(num);
		this->set_val(this->real()*temp->real()-this->imag()*temp->imag(),this->real()*temp->real()+this->imag()*temp->imag());
		delete temp;
		return this;
	}
	else{
