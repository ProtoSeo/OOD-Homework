...
Number *Integer::add(Number * num) {
    ...
    if (num->types() == INTEGER) {
		Integer* temp = new Integer(((this->value) + (dynamic_cast<Integer*>(num))->val()));
   		delete this;
		return temp;
	} else if (num->types() == FLOAT) {
		Float* temp = new Float((double)(this->value)+ (dynamic_cast<Float*>(num))->val());
		delete this;
		return temp;
    } else if (num->types() == COMPLEX) {
		Complex* temp = new Complex((double)(this->value)+(dynamic_cast<Complex*>(num))->val().first,(dynamic_cast<Complex*>(num))->val().second);
		delete this;
		return temp;
    } else {
		return nullptr;
...
Number *Integer::mul(Number * num) {
    ...
    if (num->types() == INTEGER) {
		...
		Complex* temp = new Complex((double)(this->value) * (dynamic_cast<Complex*>(num))->val().first,(double)(this->value) * (dynamic_cast<Complex*>(num))->val().second);
		delete this;
		return temp;
    } else {
		return nullptr;
