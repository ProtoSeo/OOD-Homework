...
Float::Float(const std::string& str) {
     ...
     */
	double _value = parse_string(str);
	this->value = _value;
}
...
Number *Float::add(Number * num) {
	...
	if(num->types() == INTEGER){
		Integer* temp = dynamic_cast<Integer*>(num);
		this->set_val((double)(temp->val())+this->val());
		delete num;
		return this;
	}
	else if(num->types() == FLOAT){
		Float* temp = dynamic_cast<Float*>(num);
		this->set_val(temp->val()+this->val());
		delete num;
		return this;
	}
	else if(num->types() == COMPLEX){
		Complex* temp = dynamic_cast<Complex*>(num);
		temp->set_val(temp->val().first + this->val(),temp->val().second);
		delete this;
		return temp;
	}
	else{
...
Number *Float::sub(Number * num) {
	...
	if(num->types() == INTEGER){
		Integer* temp = dynamic_cast<Integer*>(num);
		this->set_val(this->val()-(double)(temp->val()));
		delete num;
		return this;
	}
	else if(num->types() == FLOAT){
		Float* temp = dynamic_cast<Float*>(num);
		this->set_val(this->val()-temp->val());
		delete num;
		return this;
	}
	else if(num->types() == COMPLEX){
		Complex* temp = dynamic_cast<Complex*>(num);
		temp->set_val(this->val() - temp->val().first,-temp->val().second);
		delete this;
		return temp;
	}
	else{
...
Number *Float::mul(Number *num) {
	...
	if(num->types() == INTEGER){
		...
		Complex* temp = dynamic_cast<Complex*>(num);
		temp->set_val(this->val() * temp->val().first,this->val() * temp->val().second);
		delete this;
		return temp;
	}
	else{
